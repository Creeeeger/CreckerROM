...
def resolve_manifest_file(parts_dir: str, filename: str, field: str) -> str:
    if not filename or os.path.basename(filename) != filename or filename in {".", ".."}:
        raise RuntimeError(f"Invalid {field} filename in manifest: {filename!r}")

    path = os.path.join(parts_dir, filename)
    if not os.path.isfile(path):
        raise RuntimeError(f"Manifest {field} file not found: {path}")
    return path
...
def load_manifest_gaps(
        parts_dir: str,
        manifest: dict | None,
        ranges: list[tuple[str, int, int]],
) -> dict[tuple[int, int], str]:
    if manifest is None or "gaps" not in manifest:
        return {}

    raw_gaps = manifest["gaps"]
    if not isinstance(raw_gaps, list):
        raise RuntimeError("Invalid gaps in manifest: expected a JSON list.")

    expected_ranges: set[tuple[int, int]] = set()
    covered_until = 0
    for _, start, end in ranges:
        if start > covered_until:
            expected_ranges.add((covered_until, start))
        covered_until = end

    gap_files: dict[tuple[int, int], str] = {}
    for index, gap in enumerate(raw_gaps):
        if not isinstance(gap, dict):
            raise RuntimeError(f"Invalid gaps[{index}] in manifest: expected a JSON object.")
        ...
        if not isinstance(start, int) or not isinstance(end, int) or end <= start:
            raise RuntimeError(f"Invalid gaps[{index}] range in manifest: {start!r}..{end!r}")
        if size != end - start:
            raise RuntimeError(
                f"Invalid gaps[{index}] size in manifest: expected {end - start}, got {size!r}"
            )
        if not isinstance(filename, str):
            raise RuntimeError(f"Invalid gaps[{index}] filename in manifest: {filename!r}")

        gap_range = (start, end)
        if gap_range in gap_files:
            raise RuntimeError(f"Duplicate gap range in manifest: 0x{start:X}-0x{end:X}")
        gap_files[gap_range] = resolve_manifest_file(parts_dir, filename, f"gaps[{index}]")

    if set(gap_files) != expected_ranges:
        raise RuntimeError(
            f"Manifest gap ranges do not match the selected SoC layout: "
            f"expected {sorted(expected_ranges)!r}, got {sorted(gap_files)!r}"
        )
    return gap_files
...
def merge_parts(parts_dir: str, soc: str, output_path: str, final_size: int | None) -> None:
    ...
    manifest_gap_files = load_manifest_gaps(parts_dir, manifest, ranges)
    ...
    with open(output_path, "wb") as out:
        for part_name, start, end in ranges:
            ...
            if start > write_pos:
                gap_size = start - write_pos
                gap_path = manifest_gap_files.get((write_pos, start))
                if gap_path is None:
                    write_zero_padding(out, gap_size)
                    logger.info(
                        "Padded: 0x%X - 0x%X (%d bytes)",
                        write_pos, start, gap_size
                    )
                    total_padded += gap_size
                else:
                    with open(gap_path, "rb") as gap_file:
                        gap_data = gap_file.read()
                    if len(gap_data) != gap_size:
                        raise RuntimeError(
                            f"Invalid size for preserved gap: expected {gap_size} bytes, "
                            f"got {len(gap_data)} bytes ({gap_path})"
                        )
                    out.write(gap_data)
                    logger.info(
                        "Preserved gap: %s => 0x%X - 0x%X (%d bytes)",
                        gap_path, write_pos, start, gap_size
                    )
                    total_inserted += gap_size
                write_pos = start
        ...
        if manifest_tail_file:
            tail_path = resolve_manifest_file(parts_dir, manifest_tail_file, "tail_file")

            with open(tail_path, "rb") as tail_file:
                tail_data = tail_file.read()

            if manifest_source_size is not None:
                expected_tail_size = manifest_source_size - max_end
                if len(tail_data) != expected_tail_size:
                    raise RuntimeError(
                        f"Invalid tail size: expected {expected_tail_size} bytes, "
                        f"got {len(tail_data)} bytes ({tail_path})"
                    )
