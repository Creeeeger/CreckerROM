...
from binary_io import read_file, read_u32, write_u32
...
# Stage type values are stored in Samsung signer metadata; aliases normalize
# local file/stage names before signing or verification.
STAGE_TYPES = {
    "bl2": 0,
    "lk": 1,
    "el3": 2,
...
# These stages can carry a Samsung Stage-2 footer inside an AVB-wrapped image,
# so candidate discovery uses the AVB original_image_size as well as file end.
AVB_WRAPPER_STAGES = {
    "tzsw",
    "secureos",
    "secure_os",
...
def effective_verify_key_type(stage, footer_key_type):
    ...
    if stype is None:
        return 0
    # Early secure-world stages are verified with the TEE key even when the
    # stored footer key_type is reused from stock metadata.
    if 2 <= stype <= 5:
        return 0
...
def compute_digest(data, total_size, zero_epbl_checksum=False):
    ...
    # The signature covers everything before the 0x200-byte signature field;
    # EPBL also treats its header checksum as zero during signing.
...
def signer_info_offset_from_avb_original(data, original_image_size):
    # Some stock AVB-wrapped images place SignerVer metadata just before the
    # AVB original size; an inner Samsung footer may end at that metadata block.
...
def stage2_footer_candidate_sizes(stage, data):
    ...
    # Search inner, AVB-original, and file-end layouts in the same order used
    # by Samsung-wrapped boot-chain images.
