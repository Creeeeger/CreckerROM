...
from sparse_image import (
    DOWNLOAD_SIGNATURE_RECORD_SIZE,
    SIGNER_INFO_SIZE,
    SPARSE_CRC32_CHUNK,
...
def parse_download_signature_layout(path):
    ...
    # Samsung download signatures reserve the first raw output block for the
    # signature record followed by the 0x100-byte SignerInfo block.
...
def select_download_header_values(layout, rp_count, sign_type, key_type_arg, key_index_arg):
    # Keep stock key_type/key_index unless the caller explicitly overrides them;
    # the rollback count is the value intentionally changed during re-signing.
...
def read_super_first_logical_sector(path):
    header = read_sparse_header(path)
    # For super.img, Samsung also embeds SignerInfo at the first logical
    # partition sector described by Android dynamic-partition metadata.
...
def download_final_digest(path, layout, header):
    ...
    # Samsung signs SHA512(payload_digest + 0x10-byte header), not the sparse
...
def build_super_first_block(header, signer_info):
    ...
    # Logical block 0 starts with the download signature record, then the
    # SignerInfo block that Samsung tools expect for super.img.
...
def copy_with_super_signature(input_path, output_path, header, signer_info):
    ...
    # Patch the sparse stream in place conceptually: replace logical block 0 and
    # the embedded SignerInfo range without expanding the whole super image.
    ...
    # Replacing block 0 may split an existing multi-block first chunk into a
    # new one-block raw chunk plus the original remainder.
    chunk_delta = 0
